...
const nav = [
  ...
  {
    href: "/pages/dashboard/lecturers",
    label: "Lecturers",
    icon: (
      <svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    href: "/pages/dashboard/classrooms",
    label: "Classrooms",
    icon: (
      <svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentPage = nav.find((n) => pathname.startsWith(n.href))?.label ?? "Dashboard";
